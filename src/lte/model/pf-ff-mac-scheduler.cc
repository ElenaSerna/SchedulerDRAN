...
PfFfMacScheduler::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PfFfMacScheduler")
	...
	.AddAttribute ("FilesEnabled",
				   ...
				   MakeBooleanAccessor (&PfFfMacScheduler::m_generateFiles),
				   MakeBooleanChecker ())
	.AddAttribute ("MaxSubbandsUE",
				   "Maximum number of subbands available per UE",
				   UintegerValue (25),
				   MakeUintegerAccessor (&PfFfMacScheduler::m_Nsubbands_user),
				   MakeUintegerChecker<uint8_t> ())
  ;
  return tid;
}
...
PfFfMacScheduler::DoCschedLcConfigReq (const struct FfMacCschedSapProvider::CschedLcConfigReqParameters& params)
{
  ...
  std::map <uint16_t, PFCompFlowPerf_t>::iterator it;
      ...
      if (it == m_flowStatsDl.end ())
        {
          ...
          m_flowStatsDl.insert (std::pair<uint16_t, PFCompFlowPerf_t> (params.m_rnti, flowStatsDl));
          PFCompFlowPerf_t flowStatsUl;
          ...
          m_flowStatsUl.insert (std::pair<uint16_t, PFCompFlowPerf_t> (params.m_rnti, flowStatsUl));
        }
    }
...
PfFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  std::cout << "Scheduler DL" << std::endl;
  ...
  for (int i = 0; i < rbgNum; i++)
    {
      ...
      if (rbgMap.at (i) == false)
        {
          std::map <uint16_t, PFCompFlowPerf_t>::iterator it;
          std::map <uint16_t, PFCompFlowPerf_t>::iterator itMax = m_flowStatsDl.end ();
          ...
          if (itMax == m_flowStatsDl.end ())
            {
              ...
              std::cout << " any UE found " << std::endl;
			  if (m_generateFiles)
			  {
...
//		  fprintf(m_pSchedFileDet, "------------------------------------------------------------------------\n");
  ...
  std::map <uint16_t, PFCompFlowPerf_t>::iterator itStats;
