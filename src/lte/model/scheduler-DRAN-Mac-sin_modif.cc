...
schedulerDRANMac::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  std::map <uint16_t, std::vector <uint16_t> > allocationMap; // RBs map per RNTI --> nuesro usuario-subbanda
  ...
  for (std::vector<bool>::iterator it = ulRbMap.begin (); it != ulRbMap.end (); it++)
    {
      ...
      index++;
    } // Calcula el número de RBs contiguo (BW contiguo) que tiene disponible para el UL -->
  //MODIFICAR !! LO HAREMOS SEGÚN EL NÚMERO DE RBs MÁX QUE SE DEFINAN EN EL FICHERO DE ENTRADA
  ...
  for (itRach = m_rachList.begin (); itRach != m_rachList.end (); itRach++)
    {
      ...
      newRar.m_grant.m_cqiRequest = true; // Estaba en false ¿¿ PORQUÉ ?? False indica sólo CQI periódico
...
//        }
  ...
  // Actualizar m_buildDataList ya que esta lista se actualiza con la tx de los procesos HARQ que están en la cola (parte que queremos comentar)
      ...
      if (rbgMap.at (i) == false) // Hay RBG's libres
        {
          ...
          for (it = m_flowStatsDl.begin (); it != m_flowStatsDl.end (); it++)
            {
              if ((m_ffrSapProvider->IsDlRbgAvailableForUe (i, (*it).first)) == false) // true if UE can be served on i-th RB, false otherwise
                continue;
          ...
          if (itMax == m_flowStatsDl.end ())
            ...
            {
              rbgMap.at (i) = true; // Realizamos asignación RBG --> RBG no disponible
              ...
              if (itMap == allocationMap.end ()) // si no encuentra a este usuario, lo añade
                {
                  ...
                  allocationMap.insert (std::pair <uint16_t, std::vector <uint16_t> > ((*itMax).first, tempMap)); // Asignamos subbanda-UE
                }
              else
                {
