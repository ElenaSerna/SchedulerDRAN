...
schedulerDRANMac::DoCschedLcConfigReq (const struct FfMacCschedSapProvider::CschedLcConfigReqParameters& params)
{
  ...
  return;
} // Configuración del flujo (comienzo, bytes tx, lastAvgTp) --> ¿PORQUÉ UN FOR DE SIZE m_logicalChannelConfList?
...
schedulerDRANMac::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  RefreshDlCqiMaps (); // Para tener el mapa creado pero sin valores??

  int rbgSize = GetRbgSize (m_cschedCellConfig.m_dlBandwidth);
  int rbgNum = m_cschedCellConfig.m_dlBandwidth / rbgSize;
  std::map <uint16_t, std::vector <uint16_t> > allocationMap; // RBs map per RNTI --> nuestro usuario-subbandas_asignadas
  std::vector <bool> rbgMap;  // global RBGs map
  uint16_t rbgAllocatedNum = 0; // Número de rbgs ya asignados
  ...
  for (std::vector<bool>::iterator it = rbgMap.begin (); it != rbgMap.end (); it++)
    {
      if ((*it) == true )
        {
          rbgAllocatedNum++; // Obtenemos el número de rbg que han sido ya asignados
        }
    }
...
// Calculamos el máximo ancho de banda contiguo que tiene disponible en el UL
  ...
  std::cout << "Número de RNTI's " << m_rachList.size() << std::endl;
  for (itRach = m_rachList.begin (); itRach != m_rachList.end (); itRach++)
    {
      ...
      newRar.m_grant.m_cqiRequest = false; // Estaba en false ¿¿ PORQUÉ ?? False indica sólo CQI periódico
      ...
      std::cout<< "UL Grant allocated to RNTI "<< (*itRach).m_rnti << " rbStart " << rbStart << " rbLen " << rbLen << " MCS " << m_ulGrantMcs << " tbSize " << newRar.m_grant.m_tbSize << std::endl;
...
//        }
      ...
      ret.m_buildRarList.push_back (newRar); // Introducimos en el objeto que se enviará a la capa la decisión de RBs contiguos (pos y leng), mcs, tbsize, para la situación del RACH en el UL
    }
  m_rachList.clear ();
...
//  m_dlInfoListBuffered = dlInfoListUntxed;
  ...
  // Creo que hasta aquí podríamos comentarlo todo el proceso de HARQ y retx, aumentando el contador rbgAllocatedNum
...
// Asignación de RBs a usuarios
  for (int i = 0; i < rbgNum; i++)
    {
      NS_LOG_INFO (this << " ALLOCATION for RBG " << i << " of " << rbgNum);
      std::cout << " ALLOCATION for RBG " << i << " of " << rbgNum << std::endl;
      if (rbgMap.at (i) == false) // Hay RBG's libres
        {
          std::map <uint16_t, schedulerDRANFlowPerf_t>::iterator it;
          std::map <uint16_t, schedulerDRANFlowPerf_t>::iterator itMax = m_flowStatsDl.end ();
          double rcqiMax = 0.0;
          for (it = m_flowStatsDl.begin (); it != m_flowStatsDl.end (); it++) // Recorremos todos los UEs
            {
              if ((m_ffrSapProvider->IsDlRbgAvailableForUe (i, (*it).first)) == false) // true if UE can be served on i-th RB, false otherwise (based on frequency reuse policy)
                continue;
              ...
              if (sbCqi.size () > 1)
                {
                  cqi2 = sbCqi.at (1); // Porqué coge estos cqi los de las 2 primeras posiciones?? Los de las 2 primeras subbandas??
                }
                  ...
                  if (LcActivePerFlow ((*it).first) > 0)
                    {
                      ...
                      double rcqi = achievableRate / (*it).second.lastAveragedThroughput; // Nuestra métrica Rij
                      ...
                      std::cout << " RNTI " << (*it).first << " MCS " << (uint32_t)mcs << " achievableRate " << achievableRate << " avgThr " << (*it).second.lastAveragedThroughput << " Métrica " << rcqi << std::endl;

                      if (rcqi > rcqiMax) // Busca usuario con métrica más alta
                        {
                          rcqiMax = rcqi;
                          itMax = it;
                        }
          ...
          if (itMax == m_flowStatsDl.end ())
            ...
            {
              rbgMap.at (i) = true; // Realizamos asignación RBG --> Marcamos el RBG como no disponible para el siguiente usuario
              std::map <uint16_t, std::vector <uint16_t> >::iterator itMap;
              itMap = allocationMap.find ((*itMax).first); // Dentro del vector de asignaciones RBs-UEs busca al UE (a través del RNTI) con métrica más alta que se ha obtenido
              if (itMap == allocationMap.end ()) // si no encuentra a este usuario, lo añade
                {
                  // insert new element
                  std::vector <uint16_t> tempMap;
                  tempMap.push_back (i); // Añade el RBG a un mapa temporal para posteriormente añadirlo al mapa de asignaciones RBs-UEs
              ...
              std::cout << "UE assigned " << (*itMax).first << "in subband " << i << std::endl;
            }
        } // end for RBG free
    } // end for RBGs
...
// HASTA AQUÍ ASIGNACIÓN DE RBs
  ...
  while (itMap != allocationMap.end ())
    {
      ...
      uint16_t RgbPerRnti = (*itMap).second.size (); // Obtiene el número de subbandas que han sido asignadas al usuario
      std::map <uint16_t,SbMeasResult_s>::iterator itCqi;
      ...
      if (itCqi != m_a30CqiRxed.end ())
        {
          for (uint16_t k = 0; k < (*itMap).second.size (); k++)
            {
              if ((*itCqi).second.m_higherLayerSelected.size () > (*itMap).second.at (k)) // Tiene que haber tantos CQI-subbanda elementos como número de RBs tenga asignados ese usuario
                {
                  ...
                  for (uint8_t j = 0; j < nLayer; j++)
                    {
                      if ((*itCqi).second.m_higherLayerSelected.at ((*itMap).second.at (k)).m_sbCqi.size () > j) // ¿¿ QUÉ SIGNIFICA QUE EL TAMAÑO DE LOS CQI-SB SEAN MAYORES QUE EL NÚMERO DE CAPAS DEL MODO DE TX ??
                        {
  ...
  m_schedSapUser->SchedDlConfigInd (ret); // Envia a la capa MAC la decisión tomada de Scheduling sobre los datos
