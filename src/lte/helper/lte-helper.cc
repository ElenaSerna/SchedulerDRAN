...
////AÑADIDO POR MI////
double LteHelper::GetLossDl (Ptr<MobilityModel> a, Ptr<MobilityModel> b)
{
	Ptr<HybridBuildingsPropagationLossModel> model = m_downlinkPathlossModel->GetObject<HybridBuildingsPropagationLossModel>();
	return fabs(model->DoCalcRxPower(0, a, b));
}

double LteHelper::GetLossUl (Ptr<MobilityModel> a, Ptr<MobilityModel> b)
{
	Ptr<HybridBuildingsPropagationLossModel> model = m_uplinkPathlossModel->GetObject<HybridBuildingsPropagationLossModel>();
	return fabs(model->DoCalcRxPower(0, a, b));
}


Ptr<HybridBuildingsPropagationLossModel> LteHelper::GetHybridBuildings()
{
	return m_downlinkPathlossModel->GetObject<HybridBuildingsPropagationLossModel>();
}

Ptr<SpectrumPropagationLossModel> LteHelper::GetFadingModule ()
{
	return m_fadingModule;
}
