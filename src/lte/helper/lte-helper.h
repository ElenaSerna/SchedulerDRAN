...
public:
  ...
  ////AÑADIDO POR MI///
  double GetLossDl(Ptr<MobilityModel> a, Ptr<MobilityModel> b);
  double GetLossUl(Ptr<MobilityModel> a, Ptr<MobilityModel> b);
  Ptr<HybridBuildingsPropagationLossModel> GetHybridBuildings();
  Ptr<SpectrumPropagationLossModel> GetFadingModule ();
...
public:
  NetDeviceContainer InstallClusterEnbDevice (NodeContainer c, uint16_t NumberOfClustes, uint16_t NodesPerCluster);
